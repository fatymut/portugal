...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_relation'])) {
    ...
    if (preg_match('/^[a-f0-9]{24}$/i', $relationId)) {
        $relationId = new MongoDB\BSON\ObjectId($relationId);
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_individu'])) {
    if ($id) {
        // Supprimer toutes les relations liées à cette personne
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_relation'])) {
    ...
    if ($type === 'parent_enfant') {
        $role = $_POST['role_parent_enfant'] ?? 'parent';
        if ($role === 'parent') {
            $relation = [
                'type' => 'parent_enfant',
                'parent' => $id,
...
/* =========================
   ...
   ========================= */
$relations = $id ? $db->relations->find([
    '$or' => [
        ['parent' => $id],
        ['enfant' => $id],
        ['personne1' => $id],
...
<?php if ($id): ?>
<!-- Bouton suppression de la personne -->
